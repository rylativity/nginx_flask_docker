...
openssl req -new -key "root-ca.key" -out "root-ca.csr" -sha256 -subj '/C=US/ST=Virginia/L=Falls Church/O=GDITPOC/CN=Sample CA'
...
openssl req -new -key "site.key" -out "site.csr" -sha256 -subj '/C=US/ST=Virginia/L=Falls Church/O=GDITPOC/CN=localhost'
